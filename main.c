...
void GPIO(void){ // configuracion inicial
    ...
    GPIOPadConfigSet(GPIO_PORTF_BASE, GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6  | GPIO_PIN_7, GPIO_STRENGTH_2MA, GPIO_PIN_TYPE_STD_WPU); // puetos, pines a utilizar, corrientes, PULLUP O PULLDOWN WPU|WPD
}
...
void moduloUno(void){           //** modulo UNO-LDR.
    while(pos1 <  miTiempo){                                                                    // ciclo de aprox 1s. para detectar la senyal.
        ...
        if(moduloLDR == 224 && before == 0){                                                    // si es el primer modulo y begore = 0 ingresa.
...
void moduloDos(void){           //** modulo DOS-LDR.
    while(pos2 < miTiempo){
        ...
        if(moduloLDR == 208 && before == 0){ // MOD 2
           conteo();
        }
        if(moduloLDR == 240){
           before = 0;
...
void moduloTres(void){          //** modulo TRES-LDR.
    while(pos3 < miTiempo){
        ...
        if(moduloLDR == 176 && before == 0){ // MOD 3
           conteo();
        }
        if(moduloLDR == 240){
           before = 0;
...
void moduloCuatro(void){            //** modulo CUATRO-LDR.
    while(pos4 < miTiempo){
        ...
        if(moduloLDR == 112 && before == 0){ // MOD 4
           conteo();
        }
        if(moduloLDR == 240){
           before = 0;
...
int main (void){
    ...
    while(1){                                                   // al finalizar la secuencia el ciclo muestra el display con el valor final.
        Display();
    }
}
